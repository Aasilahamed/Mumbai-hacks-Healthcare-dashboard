...
export const HealthAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      id: '1', 
      role: 'assistant', 
      content: `Hello ${user?.name || 'there'}, I am your HashCare AI Health Companion. I'm here to help you with health questions, medication reminders, diet advice, and general wellness guidance. How can I assist you today?`, 
      timestamp: new Date() 
    }
  ]);
  ...
  const buildHealthContext = (): HealthContext => {
    return {
      age: user?.height ? parseInt(user.height) : 45, // Using height as age placeholder - in real app, add age field
      vitals: {
        heartRate: 72,
      ...
      lastAppointment: 'Tomorrow, 10:00 AM with Dr. Richards'
    };
  };
        ...
        <div className="p-4 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700 rounded-b-2xl">
                ...
                <div className="mb-3 flex flex-wrap gap-2">
                    {suggestions.map((suggestion, idx) => (
                        <button
                            ...
                            className="text-xs px-3 py-1.5 bg-slate-100 dark:bg-slate-700 hover:bg-brand-100 dark:hover:bg-brand-900/20 text-slate-700 dark:text-slate-300 rounded-lg transition-colors flex items-center gap-1.5"
                        >
            ...
            <div className="flex gap-3">
                ...
                <button 
                    ...
                    className="bg-brand-600 hover:bg-brand-700 disabled:bg-slate-300 dark:disabled:bg-slate-600 text-white rounded-xl px-4 py-3 transition-colors shadow-lg shadow-brand-500/20 disabled:shadow-none"
                >
                    {isTyping ? (
